...
function fit(self::FMDT, X::Matrix{Float64}, y, continous=None, numClasses=None, cPoints=None, ftype="triangular", trpzPrm=-1)
    """
    ...
    if !isnothing(numClasses)
        ...
        self.K = int(maximum(y) + 1)
    end

    if isnothing(continous)
    ...
    tree = self.buildtree(np.concatenate((X, y.reshape(y.shape[0], 1)), axis=1), depth=0, fSet=UniverseFuzzySet.createFuzzySet())
    self.tree = tree
